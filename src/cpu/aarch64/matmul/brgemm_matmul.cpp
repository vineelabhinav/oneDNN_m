...
status_t brgemm_matmul_t<isa>::init(engine_t *engine) {
    ...
    if (bgmmc.use_buffer_b){
        std::cout<<"brgemm_matmul.cpp:191:brgemm_matmul_t<isa>::init()"<<"\n";
...
status_t brgemm_matmul_t<isa>::execute_body(const exec_ctx_t &ctx) const {
    std::cout<<"brgemm_matmul.cpp:213:execute_body()"<<"\n";
    ...
    std::cout<<"oscales"<<oscales<<"\n";
    std::cout<<"src_zero_point: "<<src_zero_point<<"\n";
    std::cout<<"wei_zero_point: "<<wei_zero_point<<"\n";
    std::cout<<"dst_zero_point: "<<dst_zero_point<<"\n";
    std::cout<<"dst_scales: "<<dst_scales<<"\n";
...
void brgemm_matmul_t<isa>::compute_kernel(
    ...
    std::cout<<"brgemm_matmul.cpp:307:compute_kernel()"<<"\n";
    const auto &bgmmc = pd()->get_brgemm_matmul_conf();
...
void brgemm_matmul_t<isa>::copy_b_chunk_in_buffer(
    ...
    std::cout<<"brgemm_matmul.cpp:617:copy_b_chunk_in_buffer()"<<"\n";
    ...
    //std::cout<<"is_K_tail: "<<is_K_tail<<"\n";
    //std::cout<<"gemm_batch: "<<gemm_batch<<"\n";
    //std::cout<<"ctx: "<<ctx<<"\n";
    //std::cout<<"bgmmc.K_blk: "<<bgmmc.K_blk<<"\n";

    int gb = 0;
    for (; gb < gemm_batch; gb++) {
        const int k = k_start + gb * bgmmc.K_blk;
        ctx.src = (void *)brgmm_ctx.get_data_B_ptr(b_idx, k, n);
        //ctx.tr_src =   (void *)brgmm_ctx.get_data_B_ptr(ithr, gb, n_blk_idx);
	...
	//assert(isa == sve_512);
        std::cout<<"brgemm_matmul.cpp:651:before context exec call"<<"\n";
	(*copy_B_kernel_)(&ctx);
    }
    ...
    if (is_K_tail) {
        const int k = k_start + gb * bgmmc.K_blk;
        ctx.src = (void *)brgmm_ctx.get_data_B_ptr(b_idx, k, n);
         //tr_src is source address of scratchpad
        //ctx.tr_src =   (void *)brgmm_ctx.get_data_B_ptr(ithr, gb, n_blk_idx);
        ctx.tr_src =   (void *)brgmm_ctx.get_buf_B_ptr(ithr, gb, n_blk_idx);
        
        printf("tr_src is tail %f: \n", ctx.tr_src);
        ctx.compensation_ptr
                = (void *)brgmm_ctx.get_s8s8_comp_ptr(ithr, b_idx, n_blk_idx);
        ctx.current_K_start = k;
...
struct brgemm_matmul_t<isa>::brg_matmul_exec_ctx_t {
    brg_matmul_exec_ctx_t(const exec_ctx_t &ctx, const pd_t *pd,
        ...
        std::cout<<"brgemm_matmul.cpp:691:brg_matmul_exec_ctx_t()"<<"\n";
        ...
        buf_A_ptr_ = (use_buffer_a)
                ...
                : nullptr;
        //std::cout<<"buffer_b:",bgmmc.use_buffer_b;
        //std::cout<<"\n";
        ...
        std::cout<<"bgmmc.s8s8_compensation_required: "<<bgmmc.s8s8_compensation_required<<"\n";
    ...
    void init_brgemm_batch_elements_values(int ithr, int brg_batch_start,
            ...
            int n_blk_idx) const {
        std::cout<<"brgemm_matmul.cpp:961:init_brgemm_batch_elements_values()"<<"\n";
        ...
        for (int b_iter = 0; b_iter < brg_batch_iters; b_iter++) {
            ...
            //printf("addr_batch addr: %f",addr_batch[b_iter].ptr.B);
            //addr_batch[b_iter].ptr.B = get_data_B_ptr(b_idx, k, n); // data corrupted as not copied
        }
    }
    ...
    char *get_buf_B_ptr(int ithr, int k_blk_idx, int n_blk_idx) const {
        UNUSED(n_blk_idx);
        // buf_B_ptr_ area booked in scratchpad
        //std::cout<<"inside get_buf_b:  "<<bgmmc_.use_buffer_b<<"\n";
        //std::cout<<"chunk suze:"<<bgmmc_.buffer_b_chunk_sz<<"\n";

        //std::cout<<"bgmmc_.buffer_b_per_thread_sz:"<< bgmmc_.buffer_b_per_thread_sz<<"\n";
        //std::cout<<"ithr:"<<ithr<<"\n";
        //std::cout<<"k_blk_idx:"<<k_blk_idx<<"\n";

        //bgmmc_.buffer_b_chunk_sz=1024;
    ...
    int32_t *get_s8s8_comp_ptr(int ithr, int b, int n_blk_idx) const {
        ...
        std::cout<<"***inside get_s8s8_comp_ptr"<<"\n";
