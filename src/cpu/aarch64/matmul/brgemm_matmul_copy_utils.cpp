...
struct jit_brgemm_matmul_copy_b_f32_t : public jit_brgemm_matmul_copy_b_t,
    ...
    // this is called with context and gets execute
    void operator()(ctx_t *ctx) override { jit_generator::operator()(ctx); } 
    status_t create_kernel() override {
        std::cout<<"brgemm_matmul_copy_utils.cpp:1781:create_kernel()"<<"\n";
...
void jit_brgemm_matmul_copy_b_f32_t::copy_16_x_n_block(
    ...
    auto get_zmm1 = [](int reg_idx) {
        assert(reg_idx >= 0 && reg_idx < max_regs_available);
        return ZRegS(reg_idx);// ZRegS(reg_idx);
    
    };
    auto get_zmm2 = [](int reg_idx) {
        assert(reg_idx >= 0 && reg_idx < max_regs_available);
        return ZReg(reg_idx);// ZRegS(reg_idx);
    };

    auto load = [this, get_zmm1](int blk, int k, int n, opmask_t current_mask) {
        auto src_zmm = get_zmm1(blk);

        //src_zmm = static_cast<Xbyak_aarch64::ZRegS>(src_zmm); 
        //auto src_zmm_m =  src_zmm | current_mask | T_z;
        //printf("brgemm_matmul_copy_utils.cpp:16xn 1\n");
        //auto addr = EVEX_compress_addr(
          //      reg_src, k * src_stride_ + n * typesize_in_);
        std::cout<<"k: "<<k<<"\n";
        std::cout<<"src_stride_: "<<src_stride_<<"\n";
        std::cout<<"typesize_in_: "<<typesize_in_<<"\n";
        std::cout<<"n: "<<n<<"\n";

        auto addr = EVEX_compress_addr(X_DEFAULT_ADDR, X_TMP_0,
                reg_src, k * src_stride_ + n * typesize_in_);

        printf("***Addr: %f\n", addr);
        printf("***reg_src: %f\n", reg_src);
        //ld1w
        //st1w(src_zmm,current_mask / T_z, ptr(addr));
        //eor(zmm_zero.d, zmm_zero.d, zmm_zero.d);
        //mov(x0, 1);
        //dup (zmm_zero.d, x0);
         ld1w(src_zmm,current_mask / T_z, ptr(addr));
        //str(zmm_zero, ptr(addr)); //use SIMD instruction
        //ld1w(ZRegS(31), current_mask / T_z, ptr(addr));
        //st1w(ZRegS(31),P_ALL_ONE / T_z,ptr(addr));
        
       // ldr(zmm_zero,ptr(addr));
        
        //str(zmm_zero, ptr(addr));
        //str(ZReg(0), ptr(addr)); // write zero's to B data pointer
        //ldr(zmm_zero, ptr(addr));
        //ld1w(src_zmm, current_mask / T_z, ptr(addr));
        //ldr(src_zmm,ptr(addr)); //loading B data into src_zmm
    };
    ...
    // kmovw(kTail, tail_mask);
        //ptrue(kTail.d);
        set_preg(kTail.d,tail_mask, X_TMP_0, X_TMP_1);
    int iter = 0;
    std::cout<<"***nrows : "<<nrows<<"\n";
    std::cout<<"*** conf_->wei_n_blk: "<<conf_->wei_n_blk<<"\n";
    std::cout<<"*** n_blk_step: "<<n_blk_step<<"\n";
    for_(int k = 0; k < nrows; k++){
    for (int n = 0; n < conf_->wei_n_blk; n += n_blk_step) { //n_blk_step
        const dim_t tr_src_off = k * tr_src_stride_ + n * typesize_out_;
        //auto store_addr = EVEX_compress_addr(reg_tr_src, tr_src_off);
        auto store_addr = EVEX_compress_addr( X_DEFAULT_ADDR, X_TMP_0,reg_tr_src, tr_src_off);
        //const dim_t src_off = k * src_stride_ + n * typesize_out_;
        //auto store_addr1 = EVEX_compress_addr(reg_src, src_off);

        const int zero_padding = ncolumns - n;
        //std::cout<<"store: "<static_cast<uint64_t>(store_addr)<<"\n";
        //store_addr = 0;
        printf("store addr: %f\n",store_addr);
        //std::cout<<"\n";
        if (zero_padding <= 0) {
            std::cout<<"***inside padding"<<"\n";
            //ld1w(store_addr1,P_ALL_ONE / T_z, zmm_zero);
            //st1w(ZRegS(31),P_ALL_ONE / T_z,ptr(store_addr1));
            //ldr(ZReg(31), ptr(store_addr1));
             str(zmm_zero,ptr(store_addr));
            //str(zmm_zero, ptr(store_addr)); // src,dst //zmm_zero
            continue;
        }

        const opmask_t curr_msk = zero_padding < n_blk_step ? kTail : kFFFF;
        const int blk_idx = iter % max_regs_available;
        load(blk_idx, k, n, curr_msk); //loading B pointer to register
        //break;
        const auto src_zmm0 = get_zmm2(blk_idx);
        // ld1w(store_addr, P_ALL_ONE / T_z,src_zmm0);
        //mov(z5.d,P_ALL_ONE,11111111111111111111111);
        //st1w(src_zmm0,P_ALL_ONE / T_z,ptr(store_addr)); only if predicate
        //str(z5,ptr(store_addr));
        // mov(x0, 6);
        // dup (z5.d, x0);
        // std::cout<<"address: ";
        // std::cout<< static_cast<unsigned long long>(store_addr);
        //str(z5,ptr(store_addr));
        //str(src_zmm0,ptr(store_addr));
        //str(ZReg(31), ptr(store_addr));
        //ldr, str-->Zreg, ld1w,st1w-->ZregS

        //mov(x1,12);
        //str(x1, ptr(store_addr)); //src_zmm0, register to buffer
        //st1w(src_zmm0,P_ALL_ONE / T_z,ptr(store_addr));
        str(src_zmm0,ptr(store_addr));
        iter++;
    }
    //break;
...
void jit_brgemm_matmul_copy_b_f32_t::compute_k_loop(int ncolumns) {
    //assert(!"under construction");
//#if 0
    std::cout<<"*** In compute_k_loop: "<<ncolumns<<"\n";
    auto compute_uni_k_loop = [&](int unroll) {
        ...
        //sub(reg_K_iters, unroll, NONE, 5);
        //sub(reg_K_iters, reg_K_iters,unroll);
        //jmp(K_start_label, T_NEAR);
        //K_start_label.id=4;
        b(K_start_label); // unconditional branch, seperate API for conditional branch 
        L(K_end_label);
    };

    constexpr int k_unroll = 16; //16;
...
void jit_brgemm_matmul_copy_b_f32_t::generate() {
    std::cout<<"brgemm_matmul_copy_utils.cpp:1967:generate()"<<"\n";
    preamble();
    //assert(!"under construction");
//#if 0
    //vpxord(zmm_zero, zmm_zero, zmm_zero);
    //eor(zregb_zero, preg_zero, zregb_zero);
     eor(zmm_zero.d, zmm_zero.d, zmm_zero.d);
    ...
    //add_imm(reg_src, param1, GET_OFF(src),reg_src);
    // ldr(reg_src,Xbyak_aarch64::ptr(reg_src));

    LDR_IMM(reg_src, param1,GET_OFF(src)); // start addr of B in src and stored in reg_src
    //printf("src addr %f:", src);
    printf("***reg_src: %f\n", reg_src);
    ...
    //mov(XReg(31), ~0); // dst,src
    //mov(reg_tr_src, ~0); // dst,src
    //mov(reg_src, ~0);
...
struct jit_brgemm_matmul_copy_b_transposed_t
    ...
    void operator()(ctx_t *ctx) override { jit_generator::operator()(ctx); }
    status_t create_kernel() override { 
        std::cout<<"brgemm_matmul_copy_utils.cpp:2043:create_kernel()"<<"\n";
        return jit_generator::create_kernel(); }
...
status_t create_brgemm_matmul_copy_b(
        ...
        const brgemm_matmul_conf_t *conf) {
    std::cout<<"brgemm_matmul_copy_utils.cpp:2661:create_brgemm_matmul_copy_b()"<<"\n";
    ...
    if (is_B_transposed) {
        ...
        if (is_bf16 || is_f16 || conf->is_bf32) {
            ...
            std::cout<<"brgemm_matmul_copy_utils.cpp:2688:create_brgemm_matmul_copy_b()"<<"\n";
    ...
    std::cout<<"brgemm_matmul_copy_utils.cpp:2708:before create_kernel call"<<"\n";
